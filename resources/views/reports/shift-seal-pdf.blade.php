...
<head>
    <meta charset="UTF-8">
    <title>Shift Seal - {{ $date }} {{ $shift }}</title>
    <style>
        body {
...
<body>
    ...
    <div class="seal-info">
        <div class="seal-info-item">
            ...
            <div class="seal-info-value">{{ \Carbon\Carbon::parse($date)->format('l, F j, Y') }}</div>
        </div>
        <div class="seal-info-item">
            <div class="seal-info-label">Shift</div>
            <div class="seal-info-value" style="text-transform: capitalize;">{{ $shift }}</div>
        </div>
        <div class="seal-info-item">
            <div class="seal-info-label">Sealed By</div>
            <div class="seal-info-value">{{ $sealer->name }}</div>
        </div>
        <div class="seal-info-item">
            <div class="seal-info-label">Sealed At</div>
            <div class="seal-info-value">{{ now()->format('H:i:s') }}</div>
        </div>
    </div>
    ...
    <div class="summary">
        ...
        <div class="summary-card escalated">
            ...
            <div class="summary-value">{{ $summary['escalated'] }}</div>
        </div>
    </div>
    ...
    <table>
        ...
        <tbody>
            @forelse($logs as $log)
            <tr>
                <td><strong>{{ $log->activity->title }}</strong></td>
                ...
                <td>{{ $log->updater->name }} <br><small>#{{ $log->updater->employee_id }}</small></td>
                <td style="font-family: monospace;">{{ $log->updated_at_time->format('H:i:s') }}</td>
                <td>{{ $log->remark ? Str::limit($log->remark, 100) : '—' }}</td>
            </tr>
            @empty
            <tr>
    ...
    <div class="footer">
        <div class="footer-item">
            <strong>Seal ID:</strong> {{ $date }}-{{ $shift }}<br>
        ...
        <div class="footer-item" style="text-align: right;">
            <div class="signature-line">
                ...
                <div class="signature-name">{{ $sealer->name }}</div>
                <div style="font-size: 10px;">{{ $sealer->designation }}</div>
            </div>
        </div>
    </div>
</body>
