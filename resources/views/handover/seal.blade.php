    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-amber-50 border border-amber-200 rounded-lg p-4 mb-6">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-sm text-amber-700">This shift was sealed on {{ $existingSeal->created_at->format('Y-m-d H:i') }} by {{ $existingSeal->sealer->name }}</p>
                </div>
            </div>
        </div>
        @endif
        ...
        <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
            ...
            <div class="bg-green-50 rounded-lg p-4 shadow border-l-4 border-green-500">
                ...
                <p class="text-2xl font-bold text-green-700">{{ $summary['done'] }}</p>
            </div>
            ...
            <div class="bg-red-50 rounded-lg p-4 shadow border-l-4 border-red-500">
                ...
                <p class="text-2xl font-bold text-red-700">{{ $summary['escalated'] }}</p>
            </div>
        </div>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-6 py-4 text-sm font-medium text-gray-800">
                                {{ $log->activity->title }}
                                <span class="block text-xs text-gray-500">{{ $log->activity->category }}</span>
                            </td>
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                    @if($log->status === 'done') bg-green-100 text-green-800
                                    @elseif($log->status === 'in_progress') bg-blue-100 text-blue-800
                                    @elseif($log->status === 'escalated') bg-red-100 text-red-800
                            ...
                            <td class="px-6 py-4 text-sm text-gray-700">
                                {{ $log->updater->name }}
                                <span class="block text-xs text-gray-500">#{{ $log->updater->employee_id }}</span>
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-700 font-mono">
                                {{ $log->updated_at_time->format('H:i:s') }}
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-600">
                                {{ $log->remark ? Str::limit($log->remark, 50) : '—' }}
                            </td>
                        </tr>
                        @empty
                        <tr>
                ...
                <div>
                    ...
                    <div class="space-y-2">
                        <div>
                            ...
                            <p class="font-mono text-lg font-bold text-blue-900">{{ \Carbon\Carbon::parse($date)->format('l, F j, Y') }}</p>
                        </div>
                        <div>
                            <span class="text-xs text-blue-700">Shift:</span>
                            <p class="font-mono text-lg font-bold text-blue-900 capitalize">{{ $shift }}</p>
                        </div>
                    </div>
                </div>
                <div>
                    ...
                    <div class="space-y-2">
                        ...
                        <div>
                            ...
                            <p class="font-mono font-bold text-blue-900">#{{ auth()->user()->employee_id }}</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="flex gap-4">
            <a href="{{ route('handover.index') }}" class="px-6 py-3 bg-gray-100 hover:bg-gray-200 text-gray-800 font-medium rounded-lg transition">
                ← Back to Handover
            </a>

            @if (!$existingSeal)
            <form method="POST" action="{{ route('handover.seal.store') }}" class="flex-1">
                @csrf
                <input type="hidden" name="sealed_date" value="{{ $date }}">
