...
@section('page-title', isset($user) ? 'Edit User' : 'Add Team Member')
    ...
    <div class="bg-white rounded-xl border border-gray-100 p-6">
        <form action="{{ isset($user) ? route('users.update', $user) : route('users.store') }}" method="POST">
            ...
            <div class="space-y-5">
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="employee_id" value="{{ old('employee_id', $user->employee_id ?? '') }}"
                               {{ isset($user) ? 'readonly class="bg-gray-50"' : 'required' }}
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="department" value="{{ old('department', $user->department ?? 'Applications Support') }}"
                               class="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="designation" value="{{ old('designation', $user->designation ?? '') }}" placeholder="e.g. Support Analyst"
                           class="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                ...
                <div>
                    ...
                    <select name="role" class="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                        ...
                        <option value="team_lead" @selected(old('role', $user->role ?? '') === 'team_lead')>Team Lead</option>
                ...
                <div class="border-t border-gray-100 pt-4">
                    <p class="text-xs text-gray-400 mb-3">{{ isset($user) ? 'Leave blank to keep current password' : 'Set initial password' }}</p>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Password {{ isset($user) ? '' : '*' }}</label>
            ...
            <div class="flex gap-3 mt-6">
                ...
                <a href="{{ route('users.index') }}" class="px-4 py-2.5 text-sm text-gray-600 hover:text-gray-900">Cancel</a>
            </div>
        </form>
    </div>
</div>
